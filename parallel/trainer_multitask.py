...
class MultiTaskParallelTrainer(object):
    def __init__(self, args, config, model, criterion, optimizer = None, scheduler = None):
        self.args = args
        self.config = config
        self.model = model
        self.criterion = criterion
        self.PiPool_criterion = torch.nn.MSELoss(reduction='sum')
    ...
    def train(self, train_loader, eval_loader = None, test_loader = None):
        ...
        for epoch in range(self.args.epochs):
            self.model.train()
            train_loss, train_pipool_loss = 0.0, 0.0
            for step, batch in enumerate(train_loader):
                batch = recursive_to(batch, device=self.args.device)
                pred, delta_atom_cnt_pred = self.model(batch)
                ...
                pipool_loss = self.PiPool_criterion(delta_atom_cnt_pred, batch['delta_atom_cnt'].float()) * self.args.alpha
                loss = (loss + pipool_loss) / self.args.gradient_accumulation
                #print("pred, target, loss", pred, batch['ddG'], loss)
                #print("pipool_loss", pipool_loss)
                #print("loss,", loss)

                #print('delta_atom_cnt', batch['delta_atom_cnt'])
                train_loss += loss
                train_pipool_loss += pipool_loss
            ...
            if eval_loader is not None:
                ...
                if test_loader is not None:
                    ...
                    self.accelerator.print(
                        "{}-th epoch train loss={:.4f}, train pipool loss={:.4f}, lr={}, val loss={:.4f}, test loss= {:.4f}, acc={:.4f}, spearman coef={:.4f}".
                            format(epoch, train_loss.item(), train_pipool_loss.item(), self.optimizer.param_groups[0]['lr'], val_loss.item(), test_loss.item(), acc.item(), spearman_coef))
                    if self.args.enable_wandb and self.accelerator.is_local_main_process:
                        metrics = {'train_loss': train_loss.item(), 'train_pipool_loss':train_pipool_loss.item(),
                                   'lr': self.optimizer.param_groups[0]['lr'],  'val_loss': val_loss.item(),
                                   'test_loss': val_loss.item(), 'test_acc': acc.item(), 'spearman':spearman_coef}
                        wandb.log(metrics)
                else: # test_loader is None
                    self.accelerator.print(
                        "{}-th epoch train loss={:.4f}, train pipool loss={:.4f}, lr={}, val loss={:.4f}".
                            format(epoch, train_loss.item(), train_pipool_loss.item(), self.optimizer.param_groups[0]['lr'], val_loss.item()))
                    if self.args.enable_wandb and self.accelerator.is_local_main_process:
                        metrics = {'train_loss': train_loss.item(), 'train_pipool_loss':train_pipool_loss.item(),
                                   'lr': self.optimizer.param_groups[0]['lr'],  'val_loss': val_loss.item()}
                        wandb.log(metrics)
                ...
                self.accelerator.print("{}-th epoch train loss={}, train pipool loss={:.4f}, lr={}".format(epoch, train_loss.item(), train_pipool_loss.item(), self.optimizer.param_groups[0]['lr']))
                if self.args.enable_wandb and self.accelerator.is_local_main_process:
                    metrics = {'train_loss': train_loss.item(), 'train_pipool_loss':train_pipool_loss.item(), 'lr': self.optimizer.param_groups[0]['lr']}
                    wandb.log(metrics)
                if self.scheduler is not None and self.args.lr_scheduler_type == 'Plateau':
    ...
    def test(self, test_loader):
        ...
        self.accelerator.print("correct", correct)
        self.accelerator.print("total test", num_instances)
