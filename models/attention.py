...
class GeometricAttention(nn.Module):
    ...
    def __init__(self, node_feat_dim, pair_feat_dim, spatial_attn_mode='CB', value_dim=16, query_key_dim=16, num_query_points=8, num_value_points=8, num_heads=12):
        ...
        if spatial_attn_mode == 'CB':
            self.out_transform = nn.Linear(
                ...
                #in_features=(num_heads * value_dim) + (num_heads * pair_feat_dim),
                #in_features=(num_heads * value_dim)  + (num_heads*(3+3+1)),
                #in_features=(num_heads * value_dim),
    ...
    def forward_beta(self, R, t, p_CB, x, z, mask):
        ...
        #logits_sum = logits_node + logits_pair
        #logits_sum = logits_node + logits_spatial
        #logits_sum = logits_node
        ...
        #feat_all = self.out_transform(torch.cat([feat_spatial, feat_node], dim=-1))  # (N, L, F)
        #feat_all = self.out_transform(feat_node)  # (N, L, F)
