...
from models.se3_trans.basis import get_basis
from models.se3_trans.utils import exists, default, uniq, map_values, batched_index_select, masked_mean, \
    to_order, fourier_encode, cast_tuple, safe_cat, fast_split, rand_uniform, broadcat
from models.se3_trans.reversible import ReversibleSequence, SequentialSequence
from models.se3_trans.rotary import SinusoidalEmbeddings, apply_rotary_pos_emb
...
class AttentionSE3(nn.Module):
    def __init__(
        ...
        self.to_v = ConvSE3(fiber, hidden_fiber, edge_dim=edge_dim, pool=False, self_interaction=False,
                            fourier_encode_dist=fourier_encode_dist, num_fourier_features=rel_dist_num_fourier_features,
                            splits=splits)

        assert not (
                    linear_proj_keys and tie_key_values), 'you cannot do linear projection of keys and have shared key / values turned on at the same time'
        ...
        if linear_proj_keys:
            ...
            self.to_k = ConvSE3(fiber, hidden_fiber, edge_dim=edge_dim, pool=False, self_interaction=False,
                                ...
                                num_fourier_features=rel_dist_num_fourier_features, splits=splits)
        else:
            self.to_k = None
    ...
    def forward(self, features, edge_info, rel_dist, basis, global_feats=None, pos_emb=None, mask=None):
        ...
        for degree in features.keys():
            ...
            if self.use_null_kv:
                ...
                null_k, null_v = map(lambda t: repeat(t, 'h d m -> b h i () d m', b=q.shape[0], i=q.shape[2]),
                                     (null_k, null_v))
            ...
            if exists(global_feats) and degree == '0':
                ...
                global_k, global_v = map(lambda t: repeat(t, 'b j (h d) m -> b h i j d m', h=h, i=k.shape[2]),
                                         (global_k, global_v))
...
class OneHeadedKVAttentionSE3(nn.Module):
    def __init__(
        ...
        self.to_q = LinearSE3(fiber, hidden_fiber)
        self.to_v = ConvSE3(fiber, kv_hidden_fiber, edge_dim=edge_dim, pool=False, self_interaction=False,
                            fourier_encode_dist=fourier_encode_dist, num_fourier_features=rel_dist_num_fourier_features,
                            splits=splits)
        ...
        if linear_proj_keys:
            ...
            self.to_k = ConvSE3(fiber, kv_hidden_fiber, edge_dim=edge_dim, pool=False, self_interaction=False,
                                fourier_encode_dist=fourier_encode_dist,
                                num_fourier_features=rel_dist_num_fourier_features, splits=splits)
        else:
            self.to_k = None
    ...
    def forward(self, features, edge_info, rel_dist, basis, global_feats=None, pos_emb=None, mask=None):
        ...
        for degree in features.keys():
            ...
            if self.use_null_kv:
                null_k, null_v = map(lambda t: t[degree], (self.null_keys, self.null_values))
                null_k, null_v = map(lambda t: repeat(t, 'd m -> b i () d m', b=q.shape[0], i=q.shape[2]),
                                     (null_k, null_v))
            ...
            if exists(global_feats) and degree == '0':
                global_k, global_v = map(lambda t: t[degree], (global_keys, global_values))
                global_k, global_v = map(lambda t: repeat(t, 'b j d m -> b i j d m', i=k.shape[1]),
                                         (global_k, global_v))
...
class AttentionBlockSE3(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.attn = attention_klass(fiber, heads=heads, dim_head=dim_head, attend_self=attend_self, edge_dim=edge_dim,
                                    use_null_kv=use_null_kv,
                                    rel_dist_num_fourier_features=rel_dist_num_fourier_features,
                                    fourier_encode_dist=fourier_encode_dist, splits=splits,
                                    global_feats_dim=global_feats_dim, linear_proj_keys=linear_proj_keys,
    ...
    def __init__(
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                EGNN(fiber=fiber, edge_dim=edge_dim, hidden_dim=hidden_dim,
...
class SE3Transformer(nn.Module):
    def __init__(
        ...
        assert not (exists(num_edge_tokens) and not exists(
            edge_dim)), 'edge dimension (edge_dim) must be supplied if SE3 transformer is to have edge tokens'
        ...
        assert exists(num_degrees) or exists(
            hidden_fiber_dict), 'either num_degrees or hidden_fiber_dict must be specified'
        ...
        conv_kwargs = dict(edge_dim=edge_dim, fourier_encode_dist=fourier_encode_dist,
                           num_fourier_features=rel_dist_num_fourier_features, splits=splits)

        # causal

        assert not (
                    causal and not attend_self), 'attending to self must be turned on if in autoregressive mode (for the first token)'
        self.causal = causal
        ...
        if use_egnn:
            self.net = EGnnNetwork(fiber=fiber_hidden, depth=depth, edge_dim=edge_dim, hidden_dim=egnn_hidden_dim,
                                   coor_weights_clamp_value=egnn_weights_clamp_value, feedforward=egnn_feedforward)
        else:
            ...
            for ind in range(depth):
                ...
                layers.append(nn.ModuleList([
                    AttentionBlockSE3(fiber_hidden, heads=heads, dim_head=dim_head, attend_self=attend_self,
                                      edge_dim=edge_dim, fourier_encode_dist=fourier_encode_dist,
                                      rel_dist_num_fourier_features=rel_dist_num_fourier_features,
                                      use_null_kv=use_null_kv, splits=splits, global_feats_dim=global_feats_dim,
                                      linear_proj_keys=linear_proj_keys, attention_klass=attention_klass,
                                      tie_key_values=tie_key_values, norm_gated_scale=norm_gated_scale),
        ...
        self.norm = NormSE3(fiber_out, gated_scale=norm_gated_scale, nonlin=nn.Identity()) if (
                                                                                                          norm_out or reversible) and exists(
            fiber_out) else nn.Identity()
    ...
    def forward(
        ...
        if exists(self.pos_emb):
            assert feats.shape[
                       1] <= self.num_positions, 'feature sequence length must be less than the number of positions given at init'
            pos_emb = self.pos_emb(torch.arange(feats.shape[1], device=feats.device))
            feats += rearrange(pos_emb, 'n d -> () n d')

        assert not (self.attend_sparse_neighbors and not exists(
            adj_mat)), 'adjacency matrix (adjacency_mat) or edges (edges) must be passed in'
        assert not (self.has_edges and not exists(
            edges)), 'edge embedding (num_edge_tokens & edge_dim) must be supplied if one were to train on edge types'
        ...
        assert set(map(int, feats.keys())) == set(
            range(self.input_degrees)), f'input must have {self.input_degrees} degree'
        ...
        if exists(neighbor_mask):
            ...
            if max_neighbors > neighbors:
                print(
                    f'neighbor_mask shows maximum number of neighbors as {max_neighbors} but specified number of neighbors is {neighbors}')
        ...
        total_neighbors = int(
            min(total_neighbors, n - 1))  # make sure total neighbors does not exceed the length of the sequence itself
        ...
        x = self.net(x, edge_info=edge_info, rel_dist=neighbor_rel_dist, basis=basis, global_feats=global_feats,
                     pos_emb=rotary_pos_emb, mask=_mask)
...
class SE3Encoder(nn.Module):
    def __init__(self, feat_dim, depth = 2, use_chain_feat= False):
        super(SE3Encoder, self).__init__()
        self.residual_type_emb = nn.Embedding(21, int(feat_dim / 2))
        self.atom_type_emb = nn.Embedding(14, int(feat_dim / 4)) if use_chain_feat else nn.Embedding(14, int(feat_dim / 2))
        self.chain_type_emb = nn.Embedding(10, int(feat_dim / 4)) if use_chain_feat else None
        #self.residue_crg_emb = nn.Embedding(3, 8)
        #self.residue_phys_emb = nn.Linear(2, 8)
        self.use_chain_feat = use_chain_feat

        self.se3_transformer = SE3Transformer(dim=feat_dim, depth=depth, num_degrees=2, heads=1, dim_head=16)
    ...
    def forward(self, aa, pos14, atom_mask, chain_seq = None):
        ...
        if self.use_chain_feat:
            ...
            #feats = torch.cat([residual_feat, atom_feat, phys_feat, crg_feat], dim=-1)
        #print("feats", feats.shape)
        #print(mask, mask.shape)

        feats = self.se3_transformer(feats=feats, coors=coors, mask=mask)

        mask_out = mask[:, :, None].expand_as(feats)
        #print(mask_out, mask_out.shape)
        feats = torch.where(mask_out, feats, torch.zeros_like(feats))
        feats = feats.view(N, L, 14, -1).sum(dim=-2)
        return feats
