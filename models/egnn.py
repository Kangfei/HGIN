...
from models.atom import AtomEncoder, AtomEncoderWithPhysChem
import math
# helper functions
...
class SimpleAttention(nn.Module):
    ...
    def __init__(self, n_expert, n_hidden, v_hidden):
        ...
        self.w1 = nn.Parameter(torch.FloatTensor(n_hidden, v_hidden))
        self.w2 = nn.Parameter(torch.FloatTensor(n_expert, n_hidden))
        self.value = nn.Linear(v_hidden, int(v_hidden / n_expert))
    ...
    def forward(self, x):
        ...
        support = F.tanh(self.w1.matmul(x.permute(0,1,3,2))) # (D, V) * (N, L, V, 14) -> (N, L, D, 14)
        att = F.softmax(self.w2.matmul(support), dim = 3) # (R, D) * (N, L, D, 14) -> (N, L, R, 14)
        output = att.matmul(self.value(x)).view(N, L, -1) #  (N, L, R, 14) *  (N, L, 14, V) -> (N, L, R * V)
        return output


class SimpleReadoutAttention(nn.Module):
    """
    Simple Attention layer
    """
    def __init__(self, n_expert, n_hidden, v_hidden):
        super(SimpleReadoutAttention, self).__init__()
        self.n_expert = n_expert
        self.n_hidden = n_hidden
        self.v_hidden = v_hidden
        self.w1 = nn.Parameter(torch.FloatTensor(n_hidden, v_hidden))
    ...
    def forward(self, x):
        ...
        """
        N, L = x.shape[0], x.shape[1]
        support = F.tanh(self.w1.matmul(x.permute(0, 2, 1))) # (D, V) * (N, V, L) -> (N, D, L)
        att = F.softmax(self.w2.matmul(support), dim = 2) # (R, D) * (N, D, L) -> (N, L)
        output = att.matmul(x).view(N, -1) #  (N, L, R) *  (N, L, V) -> (N, R * V)
        return output
...
class EGNN(nn.Module):
    ...
    def forward(self, feats, coors, edges = None, mask = None, adj_mat = None):
        ...
        rel_coors = rearrange(coors, 'b i d -> b i () d') - rearrange(coors, 'b j d -> b () j d') # [batch_size, num_node, num_node, 3]
        rel_dist = (rel_coors ** 2).sum(dim = -1, keepdim = True) # [batch_size, num_node, num_node, 1] compute the relative distance
        ...
        m_ij = self.edge_mlp(edge_input) # [batch_size, num_node, num_node, m_hid]

        if exists(self.edge_gate):
            m_ij = m_ij * self.edge_gate(m_ij) # [batch_size, num_node, num_node, m_hid]
        ...
        if exists(self.coors_mlp):
            coor_weights = self.coors_mlp(m_ij) # [batch_size, num_node, num_node, 1]
            coor_weights = rearrange(coor_weights, 'b i j () -> b i j')  #[batch_size, num_node, num_node]
...
class EGNNEncoderV1(nn.Module):
    def __init__(self, feat_dim, depth, use_chain_feat= False):
        super(EGNNEncoderV1, self).__init__()
        ...
        self.global_tokens = nn.Parameter(torch.randn(4, feat_dim))
        self.layers = nn.ModuleList([])
        self.global_attn = GlobalLinearAttention(dim=feat_dim, heads=8,
                                      dim_head=64)
        for _ in range(depth):
            self.layers.append(EGNN(feat_dim, norm_feats=True, update_feats=True, update_coors=False))
    ...
    def forward(self, aa, pos14, atom_mask, phys, crg, chain_seq = None):
        ...
        if exists(self.global_tokens):
            global_tokens = repeat(self.global_tokens, 'n d -> b n d', b=feats.shape[0])
        feats, global_tokens = self.global_attn(feats, global_tokens, mask=mask)

        for egnn in self.layers:
            feats, coors = egnn(feats=feats, coors=coors, mask=mask)
...
class EGNNEncoderV2(nn.Module):
    def __init__(self, feat_dim, depth, use_chain_feat= False):
        super(EGNNEncoderV2, self).__init__()
        #self.atom_encoder = AtomEncoder(feat_dim=feat_dim, use_chain_feat=use_chain_feat)
        self.atom_encoder = AtomEncoderWithPhysChem(feat_dim=feat_dim, use_chain_feat=use_chain_feat)

        self.layers = nn.ModuleList([])
        for _ in range(depth):
            self.layers.append(EGNN(feat_dim, norm_feats=True, update_feats=True, update_coors=False))
            #self.layers.append(EGNN(feat_dim, norm_feats=True, update_feats=True, update_coors=False, num_nearest_neighbors =200))

    def forward(self, aa, pos14, atom_mask, phys=None, crg= None, chain_seq = None):
        """
        Args:
            aa:           (N, L).
            pos14:        (N, L, 14, 3).
        ...
        """
        N, L = aa.size()
        feats, coors, mask = self.atom_encoder(aa, pos14, atom_mask, phys, crg)
        for egnn in self.layers:
            feats, coors = egnn(feats=feats, coors=coors, mask=mask)
        ...
        #print(mask_out, mask_out.shape)
        feats = torch.where(mask_out, feats, torch.zeros_like(feats))
        #feats = feats.view(N, L, 14, -1)[:, :, 0:5, :].sum(dim= -2)
        feats = feats.view(N, L, 14, -1).sum(dim=-2)
        #feats = self.att_readout(feats.view(N, L, 14, -1))
        return feats


class EGNNEncoderPiPool(nn.Module):
    def __init__(self, feat_dim, depth, use_chain_feat= False):
        super(EGNNEncoderPiPool, self).__init__()
        self.atom_encoder = AtomEncoder(feat_dim=feat_dim, use_chain_feat=use_chain_feat)
    ...
    def forward(self, aa, pos14, atom_mask, phys=None, crg= None, chain_seq = None):
        ...
        """
        N, L = aa.size()
        feats, coors, mask = self.atom_encoder(aa, pos14, atom_mask)
        for egnn in self.layers:
            feats, coors = egnn(feats=feats, coors=coors, mask=mask)
        ...
        feats = torch.where(mask_out, feats, torch.zeros_like(feats))
        #feats = feats.view(N, L, 14, -1)[:, :, 0:5, :].sum(dim= -2)
        res_feats = feats.view(N, L, 14, -1).sum(dim=-2)
        return res_feats, feats
...
class EGNNEncoder(nn.Module):
    def __init__(self, feat_dim, depth, use_chain_feat= False):
        ...
        self.layers = nn.ModuleList([])
        #self.att_readout = SimpleAttention(n_hidden=feat_dim, v_hidden=feat_dim, n_expert=4)
...
if __name__ == '__main__':
    ...
    feats = torch.randn(2, 48*14, 512)
    coors = torch.randn(2, 48*14, 3)
    mask = torch.ones(2, 48*14).bool()

    feats, coors = layer1(feats, coors, mask=mask)
    print(feats.shape, coors.shape)

    feats, coors = layer2(feats, coors, mask=mask)  # (1, 16, 512), (1, 16, 3)
    print(feats.shape, coors.shape)
    """
    atom_feat = torch.randn(2, 4)
    N, L = 2, 4
    aa = torch.randn(N, L)
    residual_feat = aa[:, :, None].expand(N, L, 14)
    print(residual_feat.shape)

    residual_feat = residual_feat.contiguous().view(N, -1)
    print(residual_feat.shape)